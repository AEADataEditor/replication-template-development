...
if [ -n "$CI" ]; then
    ...
    # Note: zenodo_get uses the wget library which doesn't respect environment
    # variables for disabling progress bars. The only way to disable it is to
    # monkey-patch the wget.download() function to pass bar=None.
