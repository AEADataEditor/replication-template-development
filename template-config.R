...
if (Sys.info()['sysname'] == "Linux") {
  ...
  if (file.exists("/etc/os-release")) {
    ...
    if (length(distro_id) > 0 && grepl("^(ubuntu|debian)$", distro_id)) {
      ...
      options(repos = c(CRAN = paste0("https://packagemanager.posit.co/all/__linux__/", codename, "/", posit.date)))
      ...
      options(repos = c(CRAN = paste0("https://packagemanager.posit.co/all/__linux__/rhel9/", posit.date)))
      ...
      options(repos = c(CRAN = paste0("https://packagemanager.posit.co/all/__linux__/opensuse156/",posit.date)))
      message(paste0("Using binary PPM for Linux distribution: ", distro_id, " (version ", version_id, ")"))
    } else {
