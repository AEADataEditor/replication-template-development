...
- Unit tests for `tools/*.py` scripts live in `tools/tests/`, named `test_<script>.py` (e.g. `tools/jira_add_comment.py` -> `tools/tests/test_jira_add_comment.py`). Run one directly with `python3 tools/tests/test_<script>.py`. Each test file resolves the module under test via `sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))` so it can `import <script>` from `tools/` despite living one directory deeper.
- `docs/` is end-user documentation only (it is published as a MyST site; `docs/superpowers/**` is excluded from that build and is exempt from this rule). Never let testing content - manual QA/testing plans, "Testing"/"Testing plan" sections, references to `tools/tests/test_*.py` - live in a `docs/` page. When a tool's doc page under `docs/tools/**` or `docs/automations/**` needs a testing section, split it: keep usage/behavior docs in `docs/`, and put the testing plan in `tools/tests/testing-plan-<script>.md` (e.g. `docs/tools/repository/96-90-sbatch-shell.md` -> `tools/tests/testing-plan-sbatch-shell.md`). Cross-link between the two rather than duplicating content.
